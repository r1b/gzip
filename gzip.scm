(module gzip (open-gzip-compressed-input-port open-gzip-compressed-output-port)
  ...
  (define (write-gzip-header output-port)
    (begin
      ; magic bytes
      ...
      ; zlib uses compression level "6" so no XFL flags
      (write-string "\x00" #f output-port)
      ; unknown os
  ...
  (define (open-gzip-compressed-output-port #!optional (output-port (current-output-port)))
    (let ((deflate-port (open-zlib-compressed-output-port output-port window-bits: -15))
          (buf "")
          (checksum (crc32 0 Z_NULL 0))
          (size 0))
      (begin
        (write-gzip-header output-port)
